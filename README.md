# Реверс-инжиниринг IoT: От UART до root-бэкдора на примере IP-камеры Aceline AIP-O4
...
- [Этап 3: Художественный фильм Гая Ричи - "Сдампили"](#этап-3-художественный-фильм-гая-ричи---сдампили)
...
Мне давно не давала покоя мысль о том, что любое более-менее "умное" IoT-устройство - это ведь целый настоящий компьютер со своей ОС, драйверами, проприетарными приложениями, периферией, разными службами, конфигами и прочим. Да, ОС обычно урезанная, а вычислительных ресурсов очень мало - но, тем не менее, это маленький Linux-компьютер.

Хотелось не только изучить железо, аккуратно упакованное в корпус, но и осмотреться в роли ```root``` внутри ОС "живого" работающего IoT-устройства.

Пройдем путь от разборки корпуса до получения устойчивого root-доступа "по воздуху", сохранив при этом всю штатную функциональность системы. А заодно узнаем, насколько (не)безопасными могут быть IoT-устройства.
...
Выбор пал на IP-камеру Aceline AIP-O4, и вот что в ней есть:
...
![Камера в сборе](img/9_камера_в_сборе.jpg)
![Стикер на корпусе](img/8_стикер_на_корпусе.jpg)
...
![Общий вид разобранной камеры 2](img/2_общий_вид.jpg)

![Общий вид разобранной камеры 1](img/1_общий_вид.jpg)

![Плата спереди](img/3_плата_спереди.jpg)
...
![Плата сзади](img/5_плата_сзади.jpg)
...
В настройках терминала (я использовал *PuTTY*) указываем COM-порт USB-to-TTL адаптера (можно посмотреть в диспетчере устройств), Сonnection type - `serial`, baudrate - `115200`. Включаем питание камеры от штатного блока питания, и... в консоль посыпался лог загрузки!
...
Из консоли U-Boot мы можем изменить параметры загрузки ядра (`bootargs`). По умолчанию система монтирует ФС и просит логин/пароль. 
Командой `printenv` посмотрим какие переменные загрузки сейчас установлены. Среди них нас интересует параметр, который сейчас выглядит как `init=/linuxrc`. 
...
## Этап 3: Художественный фильм Гая Ричи - "Сдампили"
...
![Снятие дампа с прищепкой](img/6_снятие_дампа_прошивки_с_чипа_памяти.jpg)
![Скриншот NeoProgrammer](img/6.1_снятие_дампа_прошивки_с_чипа_памяти.png)
...
Но наша системе пока всё ещё в коматозном состоянии, и в ней ничего не происходит. Поэтому, руководствуясь порядком команд из `/etc/inittab` я вручную поднял систему из-под UART, чтобы получить живое окружение:
```bash
/sbin/swapoff -a
...
Подводя итоги, мне хотелось бы снова вернуться к той мысли, которую я озвучил в самом начале: подобные IoT-устройства всегда казались мне чем-то закрытым и очень проприетарным, а детали их внутреннего устройства для меня были таинственной загадкой. Когда читал новости про то, как кого-нибудь взломали через смарт-чайник или умную розетку, я всё время задавался вопросом : `"Да как, блин, через них можно вообще хоть что-то сделать? Это ведь как-то к ним подключиться нужно в обход клиентского приложения, а потом eщё и взломать что-то по сети. Это же не компьютер, и даже не телефон, а какая-то маленькая штука с каким-то, наверное, очень своеобразным внутренним устройством."`

Изначально эта камера тоже выглядела как типичный "чёрный ящик" - устройство, которое как-то делает свою работу: снимает видео, пишет звук, куда-то это сохраняет, подключается к Wi-Fi, и всё в этом духе. Вся внутренняя кухня скрыта за удобным мобильным приложением и облачным сервисом, поэтому **как именно** оно делает свою работу - было для меня загадкой.

Но за этим корпусом оказался скрыт целый мир:
- UART на плате
- загрузчик U-Boot
- Linux-ядро
- файловые системы
...
- сетевые сервисы
- конфиги
- проприетарные вендорские приложения

И когда хоть немного начинаешь вникать в это, тогда истории про "взломали через чайник" перестают казаться чем-то загадочным и невозможным.
...
URL адрес запроса, составленный определённым образом, позволяет выйти за пределы `DocumentRoot`, и веб-сервер (который в данный случае имеет `root`-права) любезно отдаст вам совершенно любой файл из системы.
